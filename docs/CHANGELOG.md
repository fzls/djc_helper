# 更新公告
1. 启用心悦app G分相关活动（理财礼卡、周礼包、心悦猫咪）（用于获取心悦G分，可在app兑换复活币、契约等道具，目前兑换流程暂不支持，需自行每日点开心悦app去兑换，或者使用auto.js脚本去每日定期自动操作）
2. 接入三月的助手编年史活动（未正式开放，无法领取游戏道具奖励）
3. 移除若干过期的活动
4. 部分活动的登录网页改为直接访问对应登录框的页面，加快打开速度，避免加载不必要的活动页面，同时可以简化找到登录框和判定登录成功的流程
5. 使用app模式打开chrome，界面更加简洁
6. 封装打开url的函数，检测启动chrome后首次访问页面失败而地址栏仍为data:，继而卡在这一步的情况，并重试
7. 需要重新登录时在tab title处显示登录信息，不再弹窗
8. 启动时设置窗口初始大小参数
9. 非ams活动也增加活动时间描述，方便看各个活动的时间
10. 新增带GUI的配置工具
11. 如需购买自动更新DLC或按月付费，请查阅目录中【付费指引v20210209.docx】(日期可能随版本变动)
