# 更新公告
1. 适配闪光杯第三期（暂不开放）
2. 过滤参数加下兼容，当解析出错时，打印日志并返回原始url
3. 渲染url参数的时候，需要将value进行url encode，避免用户配置中包含&等特殊符号时，后续逻辑出错
4. 新增史诗之路活动的指引（太小了，只提示网页，不实际实现）
5. 新增dnf漂流瓶活动
6. 新增马杰洛的规划活动
7. 增加dnf双旦活动
8. 当添加了多个活动的版本发布时，弹出一条恰饭信息
9. 增加提示拉取点赞内容流程
10. 抽取通用指引逻辑，并在命令行额外打印日志
11. 过滤未填充的参数，避免某些参数影响其他请求
12. 推荐使用使用autojs来自动完成助手编年史中的助手app相关任务、微信签到和2020DNF嘉年华派送好礼、hello语音活动等目前小助手无法直接支持的流程
13. 具体操作可参考[脚本源码](https://github.com/fzls/autojs) 以及 [演示视频](https://www.bilibili.com/video/BV1az4y1k7bH/)
