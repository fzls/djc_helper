# 更新公告
1. 检查更新时临时启用代理
2. 增加无视系统代理的配置，并默认无视代理，方便在开启代理的时候仍能正常使用小助手
3. 配置工具增加集卡最后一天抽光所有卡片的选项
4. 按照助手上显示的活动时间修改集卡结束时间
5. qq空间系的几个活动增加异常保护
6. 调整付费提示，避免歧义
7. 获取qq列表时，跳过未启用的账号列表
8. 调整dlc未购买提示
9. dlc查询资格函数增加返回是否查询成功，用于判定是查询成功有资格，还是服务暂不可用而全部放行
10. 在服务不可用而全部放行的情况下，若本地没有dlc，不尝试自动下载，而是直接跳过
11. 完善测试用例
12. 增加接入Coveralls
