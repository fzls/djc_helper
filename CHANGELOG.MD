...
3. urllib改回最新，不然部分电脑好像会出更奇怪的问题
4. 配置工具增加心悦特权专区兑换装备提升礼盒
5. 重构配置工具界面，全面支持折叠控件，使界面结构看上去更直观
6. 调整配置工具配置顺序，将更重要的配置往前挪
7. 当有账号配置时，启动时默认聚焦首个账号配置
8. 非自动登录模式时，自动折叠账号密码框
9. 配置工具中不再支持手动登录选项
10. 手游名称默认填为 无 ，需要更多聚豆的人自行填写
11. 修复礼包达人的手游名称获取失败的问题
12. 移除部分不再用到的配置
