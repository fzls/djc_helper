...
5. 新增微信公众号抽奖提示
6. 更新漫画活动集数
7. 兼容使用vpn时可能会报ValueError: check_hostname requires server_hostname的问题
8. 使用折叠控件调整道聚城兑换的界面
9. dlc操作失败时尝试更新dlc版本后重试，最多尝试3次
10. 调整若干文案
11. 通用异常消息提取中兼容配置文件无法正确读取的情况
12. 重构最大化相关函数，使最大化和最小化互不影响。同时允许falg为文件或目录均可
13. 重写清除登录信息的逻辑，仅清除登录相关内容，而不改动其他cache部分
