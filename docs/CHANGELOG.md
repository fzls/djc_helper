# 更新公告
1. 修复自动更新dlc启动失败的问题（请手动下载最新版本本体）
2. 由于QQ空间的活动经常不太稳定，允许其检查失败，让其他不依赖于QQ空间pskey的活动能继续跑
3. 重构检查p_skey是否过期的流程
4. gameinfo改为lazyload，从而不需要该文件时不必因为引用而导致报错
5. 重构制作补丁流程，旧版本列表改为从网盘获取，并按需下载、解压相应旧版本
