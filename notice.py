...
def main():
    ...
    # note: 在这里添加公告
    title = "编年史不自动领任务经验"
    message = """部分朋友反馈最近编年史不能自动领取任务经验，经排查问题如下
16.8.3及以前：接口发生变更导致无法领取
16.8.4开始：新接口必须要设置【助手token】和【唯一角色ID】，导致无法领取

总结：
现在需要【助手token】/【唯一角色ID】才能完成的编年史相关操作至少包括下面几个：
1. 领取任务奖励（除助手签到任务外，其余任务条件需自己达成，如在线1小时、分享助手周报、浏览助手视频等等）
2. 领取等级奖励
