...
class XinYueAppOperationConfig(ConfigInterface):
    def __init__(self):
        ...
        ##      形如 58 59 01 00
        ##  小黄鸟分享的请求文件，也可以使用vscode的hexdump插件来复制相关内容。打开vscode，安装hexdump插件，然后把下载的请求文件拖到vscode中，ctrl+shift+p呼出命令面板，输入hexdump，即可看到十六进制视图。跟上面的步骤一样，从58 59 01 00（XY..)那个地方一直选择到最后，然后右键选择  Copy the selection in a specific format，选择 C 的格式，然后把复制出来的内容中 { 到 } 之间的0x58, 0x59, 这些复制到下面的数组区域
