# 更新公告
1. 移除使用卡密后多余的一次弹窗
2. 网盘下载接口增加优化机制，当可配置仅当网盘中版本比本地版本新时才尝试下载，避免不必要的下载行为
3. 下载完成后再删除源文件，然后重命名临时文件，减少文件无效的时长
4. 调整压缩版本和下载文件的临时目录位置，避免冲突
5. 每次更新db文件后都修改file_created标记为true，避免因为多进程导致该值为false后再也没有设置机会，而导致first_run逻辑出错而不断弹窗的问题
