...
2. 为支持自动绑定，各个amesvr活动的操作函数同意增加extra_params不定参数，用于传入绑定所需参数
3. 移除过期的集卡活动
4. 将账号密码相关配置往前挪，更容易定位
5. 修改默认账号名和密码提示
6. 特殊处理qq视频的回包，这样结果可以更清晰
7. 新增qq视频-看江湖有翡活动
8. 额外说明一下未获取到token的链接的情况
9. 额外说明助手活动需要token
...
# v5.1.1 2021.1.9
## 自动绑定
1. 为支持自动绑定，各个amesvr活动的操作函数同意增加extra_params不定参数，用于传入绑定所需参数
