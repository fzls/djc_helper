# 更新公告
1. 重构colg活动
2. 调整使用dlc的相关逻辑，增加自动下载机制和依赖于dlc和本体互相协作的dlc自动更新机制
3. 修复因支持多进程模式后，部分逻辑处未初始化角色绑定信息的问题（如心悦固定队无法组队成功）
4. 即将过期时每天提示一次
5. 可设置提前提示付费过期的时间
6. 调整角色选择器，当无角色信息时继续显示默认提示语
7. 当存在.no_message_box文件时，不弹窗，仅刷日志
