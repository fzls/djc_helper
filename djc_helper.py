...
class DjcHelper:
    ...
    ark_lottery_sub_act_id_login = "76046_0cf16af6"  # 增加抽卡次数-每日登陆游戏
    ark_lottery_sub_act_id_share = "76040_864379f5"  # 增加抽卡次数-每日活动分享
    ark_lottery_sub_act_id_lucky = "76041_a29ebdac"  # 增加抽卡次数-幸运勇士
    ark_lottery_sub_act_id_draw_card = "76042_299ff1d6"  # 抽卡
    ark_lottery_sub_act_id_award_1 = "76043_b32977ea"  # 领取奖励-第一排
    ark_lottery_sub_act_id_award_2 = "76044_13a99233"  # 领取奖励-第二排
    ark_lottery_sub_act_id_award_3 = "76045_e04ef744"  # 领取奖励-第三排
    ark_lottery_sub_act_id_award_all = "76048_252b9178"  # 领取奖励-十二张
    ark_lottery_sub_act_id_lottery = "76047_8435b495"  # 消耗卡片来抽奖

    # note: 清空抓包数据，按f5刷新后，搜索  QueryItems  (hack: 其实就是活动链接的 最后一部分)
    ark_lottery_packet_id_card = "43854_4fb03cca"  # 查询当前卡片数目
    ...
    def majieluo_send_to_xiaohao(self, xiaohao_qq_list: list[str]) -> list[str]:
        ...
        for openid in xiaohao_qq_list:
            res = self.majieluo_op(f"赠送单个用户（发送好友ark消息）-{openid}", "141620", openid=openid, iType=iType, p_skey=p_skey)
            if int(res["iRet"]) == 0:
                results.append("赠送成功")
            else:
    ...
    def dnf_wegame(self):
        ...
        # 全民礼包
        self.dnf_wegame_op("全民礼包", "889191")

        # 四选一
        self.dnf_wegame_op("登录游戏", "891704")
        self.dnf_wegame_op("在线30分钟", "889229")
        self.dnf_wegame_op("首次分享", "889233")
        ...
        for idx in range_from_one(remainingLotteryTimes):
            res = self.dnf_wegame_op(f"{idx}/{remainingLotteryTimes} 开启补给箱-4礼包抽奖", "889237")
            if int(res.get("ret", 0)) != 0:
                break

        # 体验新副本
        for count in [5, 8]:
            self.dnf_wegame_op(f"累计通关容易难度110级地下城 {count} 次", "889245", crossTime=count)

        for count in [10, 20]:
            self.dnf_wegame_op(f"累计地下城获得Lv105史诗装备 {count} 件", "889231", getLv105=count)

        # 抽奖
        for count in [10, 30]:
            self.dnf_wegame_op(f"每日消耗疲劳值 {count} 点", "889246", use_fatigue=count)

        self.dnf_wegame_op("每周通关高级地下城-风暴逆鳞", "889249")

        totalLotteryTimes, remainingLotteryTimes = query_daily_lottery_times()
        logger.info(color("bold_yellow") + f"累计获得{totalLotteryTimes}次抽奖次数，目前剩余{remainingLotteryTimes}次抽奖次数")
        for idx in range_from_one(remainingLotteryTimes):
            self.dnf_wegame_op(f"{idx}/{remainingLotteryTimes} 次抽奖", "889253")
    ...
    def check_dnf_wegame(self, roleinfo=None, roleinfo_source="道聚城所绑定的角色"):
        self.check_bind_account(
            ...
            query_bind_flowid="889204",
            commit_bind_flowid="889203",
            roleinfo=roleinfo,
            roleinfo_source=roleinfo_source,
        )
    ...
    def maoxian(self):
        ...
        self.maoxian_op("见面礼", "890563")

        self.maoxian_op("领取任务1", "892316")
        self.maoxian_op("领取任务2", "892317")
        self.maoxian_op("领取任务3", "892318")
        self.maoxian_op("领取任务4", "892319")
        self.maoxian_op("领取任务5", "892320")
    ...
    def check_maoxian_dup(self):
        self.check_bind_account(
            ...
            query_bind_flowid="892298",
            commit_bind_flowid="892297",
        )
