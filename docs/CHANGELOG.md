# 更新公告
1. 提示：如果你16号开始几乎领不到任何奖励，请检查是否开启了【无法在道聚城绑定dnf】，若是，则请同时 取消勾选 【无法在道聚城绑定dnf】与【禁用大多数活动】这两个开关
2. 黑钻活动的页面不见了，因此未绑定的账号不再提示去绑定了
3. 修复心悦集卡活动领取亲密度奖励时提示速度过快
4. 为了对心悦集卡的概率有所了解，增加每日抽到卡片的统计，以及存量卡片的统计
5. 配置工具付费界面的QQ的自动填充不再要求配置为自动登录，只需要配置了QQ即可
