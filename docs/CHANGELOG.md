...
1. 接入新的集卡活动
2. 移除过期活动
3. 多进程登录和运行时，各进程升序等待一小段时间，避免开始时的输出混在一起
4. 增加禁用命令行快速编辑功能，从而避免电脑小白点选鼠标进入快速编辑模式时误以为是程序卡住了而不知所措（默认禁用，可配置启用）
5. 增加检测是否在pycharm中运行，并在检测到时不尝试最大化窗口，避免IDE卡住
6. 将活动概览移到外部，只显示一次
7. 登录报错处增加打印版本号，方便一眼从截图中看出版本
