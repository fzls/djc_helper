...
2. 弹出的支付宝红包图片看的不爽的，可以自行去配置工具关闭本次的开关，或者创建名为 .never_show_ask_message_box 的文件或目录，来永久关闭红包提示。表示不满可以，提出建议也接受，但是拒绝辱骂。如果你像2021.12.3某个人在群里那样辱骂我，我会将你加入黑名单，然后退还你剩余的按月付费对应的钱。
3. 新增编年史自动匹配功能
4. 个人概览增加上月编年史等级和自动匹配状态
5. 调整领取闪光周奖的逻辑，改成写死日期来尝试，并增加请求间隔，避免领取失败
6. 蓝奏云增加从备案处查到的新的域名
