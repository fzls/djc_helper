# 更新公告
1. 当在群里问（文档、视频教程、程序提示、机器人自动回复）中已经明确详细解释的问题时，请做好被采用不友好的态度对待的准备。如果无法接受这一点，可以自行退群。如果同时不想再使用，可私聊我支付宝账号，将剩余时长对应金额退回~
2. 漫画活动增加第12集
3. 调整活动概览格式，增加展示活动链接（若有）
4. 禁用快速编辑模式时提示如何复制内容
5. 读取缓存的pskey时增加异常保护，读取失败不应影响正常流程
6. 修复有账号是自动登录模式时，配置工具启动前会快速闪过几个登录窗口的问题
7. 提示付费时打开付费指引.docx
