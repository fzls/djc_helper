# 更新公告
1. 移除几个过期活动
2. 微信付款渠道已重新开放，取消按钮的点击提示
3. 修复无限提示skey过期且其他进程在登陆中的问题
4. skey同时过期的情况增加提示若一直循环，则点击清除登录状态按钮
5. first_run改用单独的字段来判定是否在指定时间内是首次运行，避免update在每次save时都会更新而导致判定出错的问题
6. 变更first run的数据库版本，废弃原有数据
7. cacheinfo也改用单独的字段保存
8. 调整部分提示中的config.toml，改为提示使用配置工具
9. 调整编年史保存数据的key名称，方便搜日志时快速定位
10. 编年史进度存盘中增加记录账号名称，方便定位自己的账号~
11. 更新付费指引中关于dlc的使用说明
