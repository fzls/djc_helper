...
4. 修复last_response_info没有正确设置的问题
5. 修复qc_IED_LOG_INFO2中无nickname时会崩溃的问题
6. 修复 刃影预约活动 活动小号会报错的问题
7. 请求过快的日志仅显示debug级别
8. 道聚城绑定补充说明
9. 管家活动补充说明
10. 增加获取对分析日志有意义的最近的调用处的函数，并应用到请求回包日志处
11. 移除周年庆的提示
12. 公告增加可配置版本需求，仅特定版本之前（不包含）显示该公告
13. Config接口增加支持解析字典值为Config接口的子字段
14. 重构first_run模块，从而避免在db.json损坏时再次判定的问题
15. 重构db模块。尽可能分布到不同文件，降低多进程同时操作时损坏的概率；同时调整接口，附带结构化信息，更加易用
16. 重构登录测试脚本
