...
2. 修复道聚城领取手游礼包时会报错的问题
3. 新增禁用分享的配置，开启后将禁用该账号基本不会发送分享消息
4. 检查pskey兼容集卡挂掉的情况
5. 接入 超级会员 活动
6. 接入 QQ空间黄钻 活动
7. 奥兹玛活动增加切换当前服务器所有达到100级的角色来尝试领取抽奖次数（需开启自动绑定活动和强制同步道聚城绑定功能）
8. 增加幸运勇士和会员关怀的角色id选择界面，降低获取角色id的门槛
9. 扫码模式下至少需要等待600秒，确保有足够时间扫码
10. 修改默认账号名
11. 配置工具新建账号时增加检查是否未填写账号名称
12. 配置工具创建账号时添加默认账号名
13. 新增强制与道聚城绑定同步的配置（默认开启），若开启，则当自动绑定的活动当前绑定角色与道聚城的不一致时，将同步修改为对应角色，从而实现快速修改活动领取角色
14. 如需购买自动更新DLC或按月付费，请使用配置工具的卡密购买功能，具体流程请查阅目录中【付费指引v20210209.docx】(日期可能随版本变动)
