...
2. 修复奥兹玛活动宝箱领取失败的问题
3. 配置工具增加autojs版的按钮
4. 接入马杰洛关怀 第八期（累计赠送30次可以领白金，但是这个不能自动操作，所以有需要的请自行去网页每天点赠送按钮（会发qq消息
5. 修复当开启超快速模式时，因wegame升级活动临时切换活动而导致其他部分活动的领奖角色不对的问题
6. 重构整体目录结构，首层目录仅放必要内容，突出重点
7. 新增dnf助手中dnf lite的autojs脚本（by 泡@842155114），有意者可自行打开https://github.com/fzls/autojs下载使用~
8. 最大化改为默认不启用
9. 窗口缓存大小及最大化和最小化改为配置项
10. 配置工具增加支持设置备份还原功能
11. 重新启用google analytics数据埋点，并增加若干功能的统计（无任何敏感信息，可自行对代码进行审计）
12. dlc增加提示购买按月付费的情况
13. 重命名config.toml.example为config.example.toml
14. 当创建了.no_sync_configs文件后，不止禁用首次使用时同步配置功能，同时禁用备份配置功能
15. 封装vuserid的设置和获取流程，兼容不存在的情况
16. 如需购买自动更新DLC或按月付费，请使用配置工具的卡密购买功能，具体流程请查阅目录中【付费指引v20210209.docx】(日期可能随版本变动)
