...
3. 配置工具中的几个dnf角色ID增加校验，确保填入的是数字id，而不是角色名称
4. 调整下登录开关的文案和排版，看上去更直观
5. 心悦战场新接口请求时增加一个判断是否请求过于频繁的流程，若是则等待三秒左右
6. 心悦战场请求加个随机0.1-1.1左右的等待时间，避免请求过快
7. 登录缓存调整类别和key，将登陆类型信息放到类别中，每一类登录的缓存分开保存
8. 增加检测心悦登录缓存是否重复，若是，则移除本地该openid对应的所有记录
9. FileNotFoundError的目标文件如果是 config.toml，添加一段额外的提示，并自动打开备份目录，方便用户复制配置文件回来
10. 如需购买自动更新DLC或按月付费，请在配置工具的【付费相关】标签页自助购买，具体流程请查阅目录中【付费指引.docx】
11. 其他改动及上述功能具体用法，详见README.MD和CHANGELOG.MD以及使用教程/使用文档.docx和教程视频
