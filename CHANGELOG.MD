...
2. 登录（若全部账密模式）、账号概览、集卡概览、自动送卡、多账号运行支持多进程模式，可通过配置开关调整（默认打开）
3. 接入 心悦app兑换功能（测试阶段）（配置方式自行查看示例配置文件，搜索 xinyue_app_operations ，配置比较复杂，因此请自行研究
4. 修复dlc赠送时长有问题的bug（精确定义为62天与2.8至实际付费结束时间之内的累积未付费时长之差）（也就是说4.11后才买的，这个等于不存在<_<）
4. 基于autojs实现预购活动，请自行点击【基于autojs实现的一些自动化脚本.html】进行了解和使用
5. 新增带GUI的配置工具
