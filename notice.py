...
def main():
    ...
    # note: 在这里添加公告
    title = "直接购买微信失败"
    message = """（【非付费】或【非使用微信支付】的朋友可无视本通知）
因支付渠道网站域名调整，配置工具中直接购买的微信渠道暂不可用，会提示【当前页面的URL未注册】
预计会持续一两天，需要等渠道方处理

对于使用【微信支付】的朋友，在此期间可以选择下列临时方案
1. 使用支付宝渠道
2. 使用卡密方案中的微信支付（具体流程参照 付费指引/方案三：基于卡密自助服务 ）
3. 使用付费指引中的付款码（具体流程参照 付费指引/方案二：付款后私聊等待手动处理 )
"""
    open_url = ""
    show_only_before_version = ""
