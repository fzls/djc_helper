...
2. 移除过期活动
3. 配置工具 增加继承旧版本配置功能
4. 调整道聚城绑定提示，更进一步傻瓜化（再问请自行删除工具和退群）
5. 配置工具 送卡QQ增加额外提示，更进一步傻瓜化（再问请自行删除工具和退群）
6. 配置工具 调整重新读取配置为自动重启
7. 配置工具 新增付费指引和使用教程的指引按钮
8. 配置工具 增加支持高分辨率屏幕（如4k屏幕），可配置开启
9. 兼容读取主进程创建的日志文件名文件时可能出错的情况，避免后面运行报错
10. 增加额外的异常说明
