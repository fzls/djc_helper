...
def main():
    ...
    # note: 在这里添加公告
    title = "编年史搭档自动匹配功能提示"
    message = """上个月增加了编年史搭档自动匹配功能，不过不少人似乎没有注意到这个功能，没有开启这个开关=、=
如果你满足下列条件：
1. 在按月付费期间
2. 上个月达到30级

那么，可以打开配置工具，在所需账号的tab中，勾选 dnf助手/是否自动匹配编年史搭档（优先级高于固定搭档） 即可
打开该开关后，若当前尚无搭档，且满足上述条件，则会尝试去与其他满足这些条件的朋友们匹配组队。

鉴于所有进入匹配队列的朋友上个月都满级了，大概率这个月也能弄到满级，这样双方都可以领到全部奖励。
"""
    open_url = ""
    show_only_before_version = ""
