...
echo 请使用网页版vscode或者下载个本地版的vscode打开config.toml，修改对应配置项，修改完成后请按ctrl+s进行保存。
echo.
echo 简单背景知识：
...
echo 3. config.toml为实际配置文件，包含大部分用户需要用到的配置项。config.example.toml为示例配置文件，包含所有公开提供的配置项
echo 4. 多账号配置时，请确保每个账号配置的name不同，否则会报错
echo 5. 很多配置项（如心悦兑换奖励列表）默认为注释状态（此时取工具设定的默认值，比如列表类配置则默认为无配置）。如果想要启用，可以在vscode中选择对应行然后按 ctrl+/ 快捷键进行取消注释。当需要注释特定配置项时，同理，选中按前述快捷键即可注释
