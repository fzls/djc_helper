...
4. 增加dnf共创投票逻辑
5. 配置工具bat中增加简要的背景知识提示
6. 调整便携版npp的注释/取消注释的快捷键为ctrl+/
7. 增加幸运角色的区服ID详细说明
8. 助手相关信息增加示例
9. 调整默认账号名称
10. 增加检测自动运行时未配置工作目录的情况
11. 展示抽卡状态时，增加提示集卡活动的截止时间
12. 增加展示抽卡所有账号汇总数据
13. 调整开启消耗所有卡片来抽奖的配置方式，改为每个集卡活动单独配置，避免共用一个开关导致在上一个活动末期开启后，下一个活动启用时忘记了配置过这个，导致最初的卡片都被拿来抽奖的问题
14. 在示例配置文件中额外暴露两个集卡活动定制化配置
15. 修正cannot_bind_dnf拼写错误
16. 道聚城配置的config中默认值值改为0，避免有人直接删除这一项，却又不填
17. 使用彩色版图标
18. 调整console中log的显示格式，使其更加简洁，固定省去26字符宽度
19. 调整一下console配色
20. 请求报错时，额外打印完整堆栈，方便得知究竟上层调用处是啥，否则直接打印异常信息时，因为在这里直接处理异常了，会导致上层的堆栈信息不打印出来
21. 请求赛利亚打工信息的时候，如果刚查询过工作日志，需要等待一会~
22. 推荐使用使用autojs来自动完成助手编年史中的助手app相关任务、微信签到和2020DNF嘉年华派送好礼、hello语音活动等目前小助手无法直接支持的流程
23. 具体操作可参考[脚本源码](https://github.com/fzls/autojs) 以及 [演示视频](https://www.bilibili.com/video/BV1az4y1k7bH/)
