...
def main():
    ...
    # note: 在这里添加公告
    title = "win11运行后桌面卡住"
    message = """如果你升级到最新版本的win11后，发现运行后桌面会卡住，可以试试下面这个方法

1. 按windows键，然后搜索 终端 ，并打开
2. 按标签页右边的 下箭头 按钮，选择 设置
3. 将第二行的 默认终端应用程序 修改为 Windows控制台主机，并点击下方的保存按钮
4. 重启电脑后再试试会不会卡住

如果还会卡住，可以试试关掉公共配置中的多进程选项
"""
    open_url = ""
    show_only_before_version = ""
