# 更新公告
1. 处理蓝奏云域名调整的问题
2. 并发调用活动时，每个活动的检查登陆流程增加异常捕获机制，避免因为检查失败而导致其他活动也无法执行
3. openid的cache的key增加账号信息，避免不同账号间混起来
4. 新增 QQ视频-爱玩 活动
5. 由于openid和access_token似乎有效期很久，增加一个缓存机制
6. 鉴于支付宝红包活动延期了，调整了下开关名称-。-PS：购买小助手的时候似乎也可以使用扫出来的红包。如果真的不想看到，就按提示关掉就好了哈哈，下次延期前不会调整的-。-
7. （小广告）前几周开始研究青龙面板白嫖京东的京豆和红包，目前已经基本把该踩的坑都踩完了，感觉还不错，两周下来领了100多元的京豆和三百多的红包（换了300QB），有兴趣的朋友可以加小群 657921324 来一起白嫖（密码 风之凌殇），目前还有不少位置，先到先得哇0-0
