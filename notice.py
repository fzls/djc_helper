...
def main():
    ...
    # note: 在这里添加公告
    title = "手机号码验证"
    message = """最近部分朋友使用自动登录可能会提示下列内容

手机号码验证
当前登录存在安全风险，请验证手机号后登录。
请通过密保手机 xxxx 获取短信验证码。

结合最近这段时间我的几个机器人也经常被冻结，很有可能是QQ的安全策略发生了改动，所以可能会突然出现这种判定登录环境异常
有点类似DNF里有时候正常玩着，某一天上去也突然安全模式一样

解决办法
1. 试试发送短信验证码后，再登录看看还会不会提示
2. 切换为【扫码登录/点击头像快速登录】一段时间，直到不再提示这个短信验证码的时候再切换回来
"""
    open_url = ""
    show_only_before_version = ""
