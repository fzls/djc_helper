...
3. 增加补充说明查询时的缓存机制，提示迟一会查询
4. 登陆增加传入参数，可设置窗口逻辑顺序，方便多个账号时可以分别展示在不同区域
5. 调整重试逻辑，分为两阶段，第一阶段在失败后等待片刻后继续尝试，总计10次，用于处理短时间内登陆过多的情况。若全部失败后则进行第二阶段，等待较长时长后再开始新一轮
6. 可配置仅启动单个实例，默认开启，确保不会因开启多个实例而导致自动更新失败
7. 查询某个账号付费信息时，合并多个来源的信息时，同时考虑关联的各个qq的信息，从而能够通过不同来源来添加账号
8. 关闭自动更新的时候增加提示
9. 标题栏增加展示付费剩余时长
