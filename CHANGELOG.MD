...
4. 增加dnf论坛签到以及兑换活动
5. 增加qq会员关怀活动
6. autojs脚本增加支持基于QQ完成预购活动每日免费抽奖的逻辑，详见 [脚本](https://github.com/fzls/autojs/blob/main/qq.js)
7. 调整绑定检查逻辑，当发现未绑定时，提醒绑定后不直接退出，而是等待其绑定后尝试下一次检测
8. qq视频活动未绑定接入通用逻辑
9. 当道聚城未绑定dnf角色时，将不尝试自动绑定
10. 调整账号提示文案及日志颜色
11. token说明分为两行，更容易阅读
12. 若.no_max_console文件存在，则不在启动时最大化窗口
13. 若工作目录存在.min_console文件，则启动时自动最小化
14. readme增加使用镜像clone的方式
15. 新增付费获取的DLC：自动更新功能（未购买则保持与之前版本的更新流程一致）。目前定价10.24元，[演示视频](https://www.bilibili.com/video/BV1FA411W7Nq)，有需要的朋友可以扫码付款后QQ私聊我发付款截图获取自动更新工具0-0
